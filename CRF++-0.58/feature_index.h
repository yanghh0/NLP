...
public:
    ...
    explicit FeatureIndex() : maxid_(0), alpha_(0), alpha_float_(0), cost_factor_(1.0), 
                              xsize_(0), check_max_xsize_(false), max_xsize_(0) {}
...
protected:
    ...
    const char *getIndex(const char *&p, size_t pos, const TaggerImpl &tagger) const;
    bool applyRule(string_buffer *os, const char *pattern, size_t pos, const TaggerImpl &tagger) const;
    ...
    std::vector<std::string>  y_;                // 训练集中标签集合，对于数据集 basenp，y_ = {B, I, O}
    std::string               templs_;
    whatlog                   what_;
};
...
{
public:
    bool open(const char *template_filename, const char *model_filename);
    bool save(const char *filename, bool emit_textmodelfile);
    bool convert(const char *text_filename, const char *binary_filename);
