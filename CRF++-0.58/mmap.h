...
{
private:
    T            *text;         // 将要打开的文件通过 mmap 映射到内存，提高读写效率
    size_t       length;
    std::string  fileName;
    whatlog what_;
...
public:
    ...
    // This code is imported from sufary, develoved by TATUO Yamashita <yto@nais.to> Thanks!
#if defined(_WIN32) && !defined(__CYGWIN__)
    bool open(const char *filename, const char *mode = "r") 
    {
        ...
        hFile = ::CreateFileW(WPATH(filename), mode1, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
        CHECK_FALSE(hFile != INVALID_HANDLE_VALUE) << "CreateFile() failed: " << filename;
    ...
    bool open(const char *filename, const char *mode = "r") 
    {
        ...
        if (std::strcmp(mode, "r") == 0)
            ...
            CHECK_FALSE(false) << "unknown open mode: " << filename;
        CHECK_FALSE((fd = ::open(filename, flag | O_BINARY)) >= 0) << "open failed: " << filename;
        CHECK_FALSE(fstat(fd, &st) >= 0) << "failed to get file size: " << filename;
        length = st.st_size;
        ...
        CHECK_FALSE(p != MAP_FAILED) << "mmap() failed: " << filename;
