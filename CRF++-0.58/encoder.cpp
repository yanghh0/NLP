            ...
            bool orthant) 
{
    ...
    for (size_t itr = 0; itr < maxitr; ++itr) {    // 开始迭代， 最大迭代次数为 maxitr，即命令行参数 -m
        for (size_t i = 0; i < thread_num; ++i) {
            thread[i].start();
        }
...
{
    EncoderFeatureIndex feature_index;
    CHECK_FALSE(feature_index.convert(textfilename, binaryfilename)) << feature_index.what();
    return true;
}
...
* @param eta 收敛阈值
* @param C cost-factor
* @param thread_num 线程数, 默认取系统的CPU数
                    ...
                    int algorithm) 
{
    ...
    CHECK_FALSE(thread_num == 1) << "This architecture doesn't support multi-thrading";
#endif

    if (algorithm == MIRA && thread_num > 1) {
        std::cerr <<  "MIRA doesn't support multi-thrading. use thread_num=1" << std::endl;
    }
...
#define WHAT_ERROR(msg) do {                                                       \
    for (std::vector<TaggerImpl *>::iterator it = x.begin(); it != x.end(); ++it)  \
        delete *it;                                                                \
    std::cerr << msg << std::endl;                                                 \
    return false; } while (0)

    CHECK_FALSE(feature_index.open(templfile, trainfile)) << feature_index.what();

    {
        ...
        size_t line = 0;
        while (ifs) {        // 开始读取训练样本
            TaggerImpl *_x = new TaggerImpl();  //_x 存放的是一句话的内容，CRF++ 官网中提到，用一个空白行将每个 sentence 隔开
    ...
    feature_index.shrink(freq, &allocator);             // 根据训练是指定的 -f 参数，将特征出现的频率小于 freq 的过滤掉
    std::vector <double> alpha(feature_index.size());   // feature_index.size() 返回的是 maxid_，即：特征函数的个数，alpha 是每个特征函数的权重，便是 CRF 中要学习的参数
    ...
    switch (algorithm) {
        case MIRA:
            if (!runMIRA(x, &feature_index, &alpha[0], maxitr, C, eta, shrinking_size, thread_num)) {
                WHAT_ERROR("MIRA execute error");
            }
            break;
        case CRF_L2:
            if (!runCRF(x, &feature_index, &alpha[0], maxitr, C, eta, shrinking_size, thread_num, false)) {
                WHAT_ERROR("CRF_L2 execute error");
            }
            break;
        case CRF_L1:
            if (!runCRF(x, &feature_index, &alpha[0], maxitr, C, eta, shrinking_size, thread_num, true)) {
...
const CRFPP::Option long_options[] = {
    ...
    {"maxiter"       ,  'm',  "100000",  "INT"       ,  "set INT for max iterations in LBFGS routine(default 10k)" },
    {"cost"          ,  'c',  "1.0"   ,  "FLOAT"     ,  "set FLOAT for cost parameter(default 1.0)"                },
    {"eta"           ,  'e',  "0.0001",  "FLOAT"     ,  "set FLOAT for termination criterion(default 0.0001)"      },
    {"convert"       ,  'C',   0      ,   0          ,  "convert text model to binary model"                       },
    {"textmodel"     ,  't',   0      ,   0          ,  "build also text model file for debugging"                 },
    {"algorithm"     ,  'a',  "CRF"   ,  "(CRF|MIRA)",  "select training algorithm"                                },
    {"thread"        ,  'p',  "0"     ,  "INT"       ,  "number of threads (default auto-detect)"                  },
    {"shrinking-size",  'H',  "20"    ,  "INT"       ,  "set INT for number of iterations variable needs to be optimal before considered for shrinking. (default 20)" },
...
int crfpp_learn(const Param &param) 
{
    ...
    if (param.get<bool>("help") || (convert && rest.size() != 2) || (!convert && rest.size() != 3)) {
        std::cout << param.help();
        return 0;
    }
    ...
    const unsigned short thread         = CRFPP::getThreadSize(param.get<unsigned short>("thread"));
