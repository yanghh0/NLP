...
// eg. Darts::DoubleArrayImpl<char, unsigned char, int, unsigned int> DoubleArray
...
private:
    ...
    // code指的是字符的编码，depth是深度，[left,right)表示这个节点在字典中的索引范围。
    struct node_t 
    {
        array_u_type_ code;
        size_t     depth;
...
next:
        ...
        for (size_t i = 0; i < siblings.size(); ++i) {
            ...
            if (!fetch(siblings[i], new_siblings)) {   // 无子节点，也就是叶子节点，代表一个词的终止且不为其他词的前缀
                array_[begin + siblings[i].code].base = value_ ?
...
public:
    ...
    // da.build(key.size(), &key[0], 0, &val[0])
