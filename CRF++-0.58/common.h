...
inline size_t tokenizeCSV(char *str, Iterator out, size_t max) 
{
    ...
    for (; str < eos; ++str) {
        ...
        if (max-- > 1) *end = '\0';  // 还不是最后一列
        *out++ = start;              // 只保存了该列字符串起始指针
        ++n;                         // 计数变量 + 1
        if (max == 0) break;
    }
    return n;  // 返回列数
...
// unsigned long/short int -> string
template <class T>
...
// define 定义得函数直接在代码处展开，append 是 string 的内置函数
// 下面定义的几个函数是为 string_buffer 类服务的
#define _ITOA(_n) do {                          \
    char buf[64];                               \
    itoa(_n, buf);                              \
    append(buf);                                \
...
#define CHECK_FALSE(condition) \
    if (condition) {} else return \
        wlog(&what_) & what_.stream_ << __FILE__ << "(" << __LINE__ << ") [" << #condition << "] "
