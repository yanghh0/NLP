...
public:
    ...
    bool openFromArray(int argc,  char** argv, const char *buf, size_t size);
    bool openFromArray(const char* arg, const char *buf, size_t size);
...
private:
    ...
    bool openFromArray(const Param &param, const char *buf, size_t size);
...
public:
    ...
    bool         open(FeatureIndex *feature_index, unsigned int nvest, unsigned velvel);
...
private:
    ...
    std::vector<std::vector <const char *> > x_;     // 代表一个句子，外部 vector 代表多行（多个词），内部 vector 代表每行的多列，具体的列用 char* 表示
    std::vector<std::vector <Node *> > node_;        // 二维数组，node_[i][j] 表示一个节点，即：第 i 个词是第 j 个 label 的点
    std::vector<std::vector<double> > penalty_;
    std::vector<unsigned short int>  answer_;        // 每个词对应的 label
    std::vector<unsigned short int>  result_;
    whatlog       what_;
    string_buffer os_;

    scoped_ptr<std::priority_queue <QueueElement*, std::vector <QueueElement *>, QueueElementComp> > agenda_;
