...
bool ModelImpl::openFromArray(const Param &param, const char *buf, size_t size) 
{
...
bool ModelImpl::openFromArray(int argc,  char** argv, const char *buf, size_t size) 
{
    Param param;
    CHECK_FALSE(param.open(argc, argv, long_options)) << param.what();
...
bool ModelImpl::openFromArray(const char* arg, const char *buf, size_t size) 
{
    Param param;
    CHECK_FALSE(param.open(arg, long_options)) << param.what();
    return openFromArray(param, buf, size);
...
bool TaggerImpl::open(FeatureIndex *feature_index, Allocator *allocator) 
{
    close();
    mode_ = LEARN;
...
bool TaggerImpl::open(FeatureIndex *feature_index, unsigned int nbest, unsigned int vlevel) 
{
    close();
    mode_ = TEST_SHARED;
    feature_index_ = feature_index;
...
bool TaggerImpl::add2(size_t size, const char **column, bool copy) 
{
    ...
    if ((mode_ == LEARN && size < xsize + 1) || ((mode_ == TEST || mode_ == TEST_SHARED)  && size < xsize)) {
...
{
    if (!agenda_.get()) {
        agenda_.reset(new std::priority_queue <QueueElement*, std::vector<QueueElement *>, QueueElementComp>);
...
bool TaggerImpl::next() 
{
    while (!agenda_->empty()) {
        ...
        for (const_Path_iterator it = rnode->lpath.begin(); it != rnode->lpath.end(); ++it) {
...
const char* TaggerImpl::parse(const char*input, size_t len1, char *output, size_t len2) 
{
...
bool TaggerImpl::parse_stream(std::istream *is, std::ostream *os) 
{
    if (!read(is) || !parse()) {
        return false;
    }
...
const char* TaggerImpl::toString(char *output, size_t len) 
{
...
Model *createModelFromArray(int argc, char **argv, const char *buf, size_t size) 
{
    ModelImpl *model = new ModelImpl();
...
Model *createModelFromArray(const char *arg, const char *buf, size_t size) 
{
    ModelImpl *model = new ModelImpl();
