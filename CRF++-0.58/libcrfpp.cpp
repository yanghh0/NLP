...
std::string WideToUtf8(const std::wstring &input) 
{
    ...
    const int result = ::WideCharToMultiByte(CP_UTF8, 0, input.c_str(), -1, input_encoded.get(),
...
crfpp_model_t* crfpp_model_from_array_new(int argc,  char **argv, const char *buf, size_t size) 
{
    CRFPP::Model *model = CRFPP::createModelFromArray(argc, argv, buf, size);
    if (!model) {
        return 0;
    }
    return reinterpret_cast<crfpp_model_t *>(model);
...
crfpp_model_t* crfpp_model_from_array_new2(const char *arg, const char *buf, size_t size) 
{
    CRFPP::Model *model = CRFPP::createModelFromArray(arg, buf, size);
    if (!model) {
        return 0;
    }
    return reinterpret_cast<crfpp_model_t *>(model);
...
{
    return reinterpret_cast<const char *>(reinterpret_cast<CRFPP::Model *>(c)->getTemplate());
}
...
const int* crfpp_next_transition_vector(crfpp_t* c, size_t i, size_t j, size_t k) 
{
    return reinterpret_cast<CRFPP::Tagger *>(c)->next_transition_vector(i, j, k);
}

const int* crfpp_prev_transition_vector(crfpp_t* c, size_t i, size_t j, size_t k) 
{
    return reinterpret_cast<CRFPP::Tagger *>(c)->next_transition_vector(i, j, k);
}
...
const char* crfpp_parse_tostr3(crfpp_t* c, const char* str, size_t len, char *ostr, size_t len2) 
{
