...
template <class T> 
static inline void read_static(const char **ptr, T *value) 
{
...
{
    std::map <std::string, std::pair<int, unsigned int> >::iterator it = dic_.find(key);
    if (it == dic_.end()) {
        dic_.insert(std::make_pair(std::string(key), std::make_pair(maxid_, static_cast<unsigned int>(1))));
        const int n = maxid_;
        maxid_ += (key[0] == 'U' ? y_.size() : y_.size() * y_.size());
        return n;
    } else {
        it->second.second++;        // 该特征出现的频次增加
        return it->second.first;
    }
    return -1;
}

bool EncoderFeatureIndex::open(const char *template_filename, const char *train_filename) 
{
...
bool EncoderFeatureIndex::openTagSet(const char *filename) 
{
    ...
    while (ifs.getline(line.get(), line.size())) {
        ...
        const size_t size = tokenize2(line.get(), "\t ", column.get(), column.size());  // 返回列数
        if (max_size == 0) {
            max_size = size;
        }
        CHECK_FALSE(max_size == size) << "inconsistent column size: " << max_size << " " << size << " " << filename;
        xsize_ = size - 1;                   // 特征的列数
        candset.insert(column[max_size-1]);  // 把该行的标签保存，set 是用来去重
    }

    y_.clear();    // 存放去重后的所有标签
    for (std::set<std::string>::iterator it = candset.begin(); it != candset.end(); ++it) {
        y_.push_back(*it);
    }
...
void EncoderFeatureIndex::shrink(size_t freq, Allocator *allocator) 
{
    ...
    std::map<int, int> old2new;   // <原来的特征id，去掉低频特征后的新id>
    int new_maxid = 0;

    for (std::map<std::string, std::pair<int, unsigned int> >::iterator it = dic_.begin(); it != dic_.end();) {
...
bool EncoderFeatureIndex::convert(const char *text_filename, const char *binary_filename) 
{
    ...
    // read header
    while (true) {
        CHECK_FALSE(ifs.getline(line.get(), line.size())) << " format error: " << text_filename;

        if (std::strlen(line.get()) == 0) {     // 空行则跳出循环
            break;
        }
    ...
    while (true) {
        ...
        if (line[0] == 'U') {
            ...
            CHECK_FALSE(true) << "unknown type: " << line.get() << " " << text_filename;
        }
    }
    ...
    while (true) {
        ...
        dic_.insert(std::make_pair(std::string(column[1]), std::make_pair(std::atoi(column[0]), static_cast<unsigned int>(1))));
    }
...
bool EncoderFeatureIndex::save(const char *filename, bool textmodelfile) 
{
    ...
    for (std::map<std::string, std::pair<int, unsigned int> >::iterator it = dic_.begin(); it != dic_.end(); ++it) {
    ...
    CHECK_FALSE(da.build(key.size(), &key[0], 0, &val[0]) == 0) << "cannot build double-array";

    std::ofstream bofs;
    bofs.open(WPATH(filename), OUTPUT_MODE);   // 生成二进制文件
...
bool DecoderFeatureIndex::openFromArray(const char *ptr, size_t size) 
{
    ...
    CHECK_FALSE(version_ / 100 == version / 100) << "model version is different: " << version_ << " vs " << version;
