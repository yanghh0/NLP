...
def learn_bpe(infile_names, outfile_name, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False):
    ...
    for f in infile_names:
        ...
        # 形如: vocab = {'low': 5, 'lower': 2, 'newest': 6, 'widest': 3}
        vocab = update_vocabulary(vocab, f, is_dict)

    # 形如: {('l', 'o', 'w</w>'): 5, ('l', 'o', 'w', 'e', 'r</w>'): 2, ('n', 'e', 'w', 'e', 's', 't</w>'): 6, ('w', 'i', 'd', 'e', 's', 't</w>'): 3}
    vocab = dict([(tuple(x[:-1])+(x[-1]+'</w>',) ,y) for (x,y) in vocab.items()])

    # 按照频率进行降序排列
    # 形如 [(('n', 'e', 'w', 'e', 's', 't</w>'), 6), (('l', 'o', 'w</w>'), 5), (('w', 'i', 'd', 'e', 's', 't</w>'), 3), (('l', 'o', 'w', 'e', 'r</w>'), 2)]
    sorted_vocab = sorted(vocab.items(), key=lambda x: x[1], reverse=True)

    # stats 形如: {('n', 'e'): 6, ('e', 'w'): 6, ('w', 'e'): 8, ('e', 's'): 9, ('s', 't</w>'): 9, ('l', 'o'): 7, ('o', 'w</w>'): 5, ('w', 'i'): 3, ('i', 'd'): 3, ('d', 'e'): 3, ('o', 'w'): 2, ('e', 'r</w>'): 2})
    ...
    with codecs.open(outfile_name, 'w', encoding='utf-8') as outfile:
        ...
        for i in range(num_symbols):
            if stats:
                # max默认迭代key，先比较2-gram的频数，如果相同再进行字符比较
